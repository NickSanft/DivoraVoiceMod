...
- A counting global allocator around **the real drain**, not a copy of it: the test calls the same function the output callback calls, and a chain replacement, a `Clear` and a voice-model install must each allocate and free **nothing**. Driving the real function is the point — a copy stays allocation-free for ever while the callback beside it regresses. Verified by putting each original defect back at the actual callback site and watching the test fail (129 allocations and 232 deallocations across the swap and the clear). It also measures the old path in the same run, so a broken allocator hook cannot let it pass by measuring nothing.
- The same for the reactive drain, including the case a copy would have grown the route `Vec` for; verified by putting the copy back and watching it fail.
- And one test that pins what is *not* fixed: `SetParam` still frees the owned key string it was sent, once per slider tick. Asserted at exactly one free, so it cannot grow quietly — and so interning the keys will fail the test and take the comments admitting it along with it.
...
- **"Speak" workspace** (sidebar → Speak): a text box, a **6-voice picker** (US + UK), Speak / Stop, a "synthesizing…" state, and a playback progress ring. Voices that aren't installed show a graceful "voice not installed" notice instead of failing.
- **On-device TTS engine.** **Kokoro-82M** (Apache-2.0, int8 ONNX, 24 kHz) for synthesis + **espeak-ng** (GPL-3.0) for text→phoneme, run as a **separate arm's-length subprocess** (never linked) so DivoraVoice's own code stays MIT.

### Architecture (additive)

- New `divora-core/src/tts/` module: `tokens.rs` (Kokoro phoneme→id mapping with `[0, …, 0]` framing + sentence/word chunking under the 510-phoneme cap; vocab loaded from the model's `config.json` at runtime — pure, unit-tested), `phonemize.rs` (espeak-ng CLI wrapper — arg build, `--path` ordering, IPA cleanup), `kokoro.rs` (the ONNX session + a compact dependency-free **`DVTS` voice pack** parser — preset style vectors only, ~3 MB vs the upstream 27 MB NPZ), and `mod.rs` (`synthesize()` → phonemize → tokenize → infer → concat, degrading to `NotInstalled` without ever touching `ort` when assets are absent, mirroring `dsp::voice_convert`).
- Backend `list_tts_voices` / `speak` / `stop_speak` commands; synthesized 24 kHz audio plays through the existing `SoundboardCommand::Play` mixer seam (`clip_id: "tts"`). New `localStorage["divora.ttsVoice"]`, `TtsVoiceInfo` wire type — both added to STABLE-SURFACE.
- **Assets** (staged on the `voice-assets-v2` release, fetched by `scripts/fetch-voice-assets.ps1`, mapped to `<resource>/tts/` by `tauri.bundle.conf.json`): `kokoro-v1.0.int8.onnx` (~88 MB), `voices-divora.bin` (~3 MB, generated by `scripts/prep-tts-voices.py`), `kokoro-config.json`, and espeak-ng (`espeak-ng.exe` + `libespeak-ng.dll` + `espeak-ng-data.zip`). Installer grows ~100 MB.

### Tests

- +22 Rust (`tts::tokens` mapping/chunking/vocab; `tts::phonemize` arg order + IPA cleanup + missing-binary gate; `tts::kokoro` `DVTS` parse/index/clamp; `tts::mod` not-installed / unknown-voice / empty-text gates + key freeze) **plus 2 `#[ignore]`d real-synthesis tests** (Kokoro infer + full espeak→audio pipeline) that pass locally against the staged assets and prove the `ort` wiring matches the Python contract. +6 frontend store, +4 api command-name, +2 E2E.
- Caught (and fixed) a real bug pre-ship: espeak's `--` end-of-options separator was emitted *before* `--path`, so espeak ran with no data dir and crashed (0xC0000005). Now `--path` precedes `--`.

### License

- README now discloses the bundled **GPL-3.0 espeak-ng** (with source URL) as a separate component — see [README](README.md#license).
