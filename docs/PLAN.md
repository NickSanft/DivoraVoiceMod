...
- **Emotion labels, emotion detection, or any UI string that names what the speaker is feeling.** "Voice reading" measures a *signal* and says so; its vocabulary is a closed list (`divora-core/src/dsp/reading.rs`) behind a test that bans Recital 18's own illustrative emotion words and second-person phrasing. Three reasons, in order of weight. (1) It would not work: the pleasant/unpleasant axis people mean by "emotion" is carried by the words, not the voice — strip prosody and keep words and a model's valence score largely survives; strip words and keep prosody and its arousal score falls to ~0.04. Voice alone measures intensity. (2) This app is a modulator: a published formant shift alone costs a four-class emotion model 18 points of accuracy (71% → 53% UAR) and neural voice conversion costs 28, and Divora's chain is harsher than either. (3) EU AI Act Recital 18 excludes "characteristics of a person's voice, such as a raised voice or whispering" from the emotion-recognition definition **unless used to infer emotions** — so the word printed on screen is the entire legal line. Same call `divora-core/src/dsp/reactive.rs` made for modulation in v1.46.0. The reading is also in-app only and never on the stream overlay: an overlay audience cannot check a reading, and a guest on the mic would be read too.
...
| post-1.0 ✓ | v1.11.0 | **Light theme**: a full light theme added as an *orthogonal* light/dark axis (`data-theme`) that composes with every Color mood (lilac / parchment / slate light palettes). Settings → Appearance **Theme** toggle; additive `TweaksState.theme`. "Deep" elements (voice core, wizard sigil disc, scrim) stay dark via new `--core-ink` / `--scrim` tokens. | Toggling Theme → Light flips the whole app to a legible light palette; it composes with each mood; the choice persists |
| post-1.0 ✓ | v1.12.0 | **In-app update check + E2E smoke tests**: a lightweight, opt-out GitHub-release update check (no telemetry — a one-way version read) surfaced in Settings → About; plus the Playwright E2E smoke harness (7 specs + a CI job) guarding the core UI on every push. Additive `divora.updateCheckEnabled`. | App shows "vX.Y.Z available → Download" when a newer release exists; `pnpm test:e2e` is green in CI |
| post-1.0 ✓ | v1.13.0 | **Setup diagnostic ("Test my setup")**: a one-click Settings → Diagnostics checklist that walks mic → output → engine → VB-Cable routing and reports pass/warn/fail per step (the #1 user-confusion point). Pure `evaluateDiagnostics` + store `runDiagnostics()` reusing existing commands; no new backend surface; restores the engine's prior run-state. | One click shows a green/amber/red checklist; a misrouted output reads "warn" with the fix; the engine is left as it was |
| post-1.0 ✓ | v1.16.0 | **Spell-circle stream overlay**: a transparent, always-on-top second Tauri window (`overlay.html` → `src/overlay.tsx`, Vite multi-entry) renders the spell circle for OBS capture, fed live state over a new `overlay:state` event. Settings toggle + background mode (transparent / chroma). No new deps/server. Additive `divora.overlay`. | Toggling the overlay shows a frameless always-on-top spell circle tracking the live voice; OBS captures it (alpha or chroma); the choice persists |
| post-1.0 ✓ | v1.15.0 | **Custom glyph casting**: record-your-own glyphs (a $1-style unistroke recognizer, `src/data/unistroke.ts`, orientation-preserving) + a Settings record pad; generalize glyph bindings so any glyph (built-in or custom) casts **any action** (preset / clip / monitor / mute / panic), reusing the v1.9 MIDI action methods. Bindings + custom glyphs now persist. | Record a glyph, bind it to a clip; drawing it on the Mixer fires the clip; built-ins can bind to any action; bindings survive a restart |
| post-1.0 ✓ | v1.14.0 | **Quick wins**: (1) **preset import** (`import_preset` command, dialog + Import button) — the counterpart to Export, saved as a unique User preset; (2) **on-disk logging** (`tracing-appender` → `%APPDATA%/DivoraVoice/logs/`) + `logs_dir` command + **Open logs folder** / **Report a problem** in Settings → About; (3) a README **Roadmap & known-limitations** section. | Import a `.json` → it appears as a User preset and previews; Open logs folder reveals a real rolling log; Report opens the issue composer |
| post-1.0 ✓ | v1.17.0 | **Text-to-speech "Speak"**: a new Speak section — type text → pick a preset **Kokoro-82M** voice (6 US/UK voices) → synthesize → play through the output, mixed with the mic via the soundboard seam. Fully on-device (Kokoro int8 via `ort`; **espeak-ng** phonemizer run as an arm's-length GPL subprocess so app code stays MIT). New `divora-core/src/tts/` (tokenizer + chunker + phonemizer + `kokoro.rs` ONNX/compact `DVTS` voice pack), `list_tts_voices`/`speak`/`stop_speak`, screen/store/nav. Assets (Kokoro + espeak-ng) staged on `voice-assets-v2`, fetched + bundled to `<resource>/tts/`. Pipeline verified end-to-end (2 ignored real-synthesis tests). Additive `divora.ttsVoice`, `TtsVoiceInfo`. | Type → pick a voice → Speak → hear that voice on the output, distinct per voice, mixed into a Discord/stream call; missing assets degrade to a graceful "not installed". |
| post-1.0 ✓ | v1.18.0 | **Speak: volume + preview-only**: a volume slider (0–200 %) and a **Preview only** toggle that routes synthesized speech to the local monitor only (you hear it, the call doesn't) via a new `SoundboardCommand::PlayMonitorOnly` + `SoundboardMixer::mix_monitor_into` seam. `speak` gains optional `gain`/`previewOnly` args (additive). Additive `divora.ttsVolume`, `divora.ttsPreviewOnly`. | Slider changes Speak loudness; Preview-only plays to your monitor but not the call; both persist. |
| post-1.0 ✓ | v1.19.0 | **Speak: Aoede preset**: a 7th bundled Kokoro voice (`af_aoede`, clear US female) — the closest stock voice to a demo reference. `voices-divora.bin` regrows 6 → 7. | Aoede appears in the picker and speaks. |
| post-1.0 ✓ | v1.20.0 | **Speak: clone your own voice (Phase 2)**: import a 20–30 s reference clip → **OpenVoice v2** (MIT) extracts a 256-d speaker embedding; Speak generates with a Kokoro base (`am_puck`) then recolors the timbre toward you. New `divora-core/src/tts/clone.rs` (realfft spectrogram + extractor/converter via `ort`), `tts::synthesize_cloned`/`extract_voice_se`, `clone_voice`/`list_cloned_voices`/`delete_cloned_voice`, "Your voices" UI, cloned-SE storage under `voices/cloned/`. Timbre-only (accent from the base). Validated (SE cosine ~0.34→~0.89). | Add a clip → it appears + auto-selects; Speak takes on your timbre, mixed into a call; delete/persist work; missing models degrade to the base voice. |
| post-1.0 ✓ | v1.21.0 | **Smaller cloning install**: the OpenVoice models (~157 MB) are no longer bundled — fetched on demand into `%APPDATA%/DivoraVoice/tts/` on first cloning use (a `ureq`-streamed download with a progress bar). Installer drops ~237 MB → ~91 MB. New `clone_models_status` / `download_clone_models` + the `clone-model-download` event; `synthesize_cloned`/`extract_voice_se` take the clone dir explicitly. (Quantization was tried first — int8 emits `ConvInteger` the CPU EP can't run, so on-demand download it is.) | Base installer small again; first clone prompts a one-time model download with progress. |
| post-1.0 ✓ | v1.22.0 | **Clones auto-match the closest base**: at clone time the app compares the reference's speaker embedding against every preset's and generates from the **best match** (gender/accent land closer) instead of a fixed `am_puck`. New `tts::pick_base_voice` (argmax SE-cosine over a bundled `base-ses.json` — 7 preset SEs, ~37 KB) + `tts::preset_short_name`; `meta.base` stores the pick; `ClonedVoiceInfo.baseName` surfaces it under each voice. | Cloning a UK/female reference starts from a UK/female base; "Your voices" reads "based on …"; falls back to `am_puck` if the data is missing. |
| post-1.0 ✓ | v1.23.0 | **Record your voice in-app**: a dry-microphone recorder captures a clean reference (pre-effects) straight into the clone pipeline — name → **Record** → **Stop & save** — no external WAV. Engine: a parallel dry-input reference ring (input-callback `push_slice`) + a second `recording_writer`; `start/stop_reference_recording` (synchronous stop finalizes the WAV before cloning). Backend `start_voice_recording`/`stop_voice_recording(name)` reuse a shared `clone_from_path`. Transient clip, deleted after. | Name a voice, record 20–30 s, Stop & save → it's cloned and selected; "Pick a clip" still imports; engine-off shows a clear hint. |
| post-1.0 ✓ (research) | spike | **Accent-preserving cloning — research done** ([docs/research/accent-preserving-cloning.md](research/accent-preserving-cloning.md)): timbre-only (OpenVoice) can't carry accent, and VC is the wrong paradigm (it keeps the *source* = Kokoro accent). The fix is **reference-conditioned zero-shot TTS**, and unlike before, permissive (Apache/MIT code+weights), CPU-runnable, ONNX-friendly, accent-preserving candidates now exist. **Verdict: conditional GO for a feasibility spike** — prototype **VoxCPM-0.5B** (Apache, RTF ~1.5 quantized) + **Chatterbox** (MIT, ready CPU ONNX), measure accent retention + CPU s/utterance + download size; keep OpenVoice as fallback. | A clone keeps the speaker's accent — pending spike confirmation of quality + CPU latency. |
| post-1.0 ✓ | v1.24.0 | **Accent-preserving cloning shipped (VoxCPM)**: the full VoxCPM-0.5B (Apache-2.0) zero-shot TTS engine ported to Rust/`ort` — new `divora-core/src/tts/voxcpm.rs` (tokenizer + VAE + prefill + autoregressive decode loop with KV cache + seeded diffusion noise + VAE decode, each parity-validated vs a Python ORT oracle), held-sessions `VoxCpmEngine`. Recorded clones (read a fixed sentence → known transcript, no ASR) store a `reference.wav` + `engine: "voxcpm"`; `speak` routes them through the engine (presets/imports unchanged). Per-channel **Q8** models (~1.6 GB) hosted on the release, downloaded on demand. End-to-end timbre cosine **0.878** (= fp32). See [docs/research/voxcpm-port-blueprint.md](research/voxcpm-port-blueprint.md). | Download the accent models → Record (read the sentence) → Speak in your own voice **and** accent; "Pick a clip" stays timbre-only OpenVoice; missing models prompt the one-time download. |

**Deferred — real-time zero-shot conversion for the live changer** (*hybrid step 2*): an any-to-reference VC model (convert toward a reference clip, no per-voice training) would unlock realistic Coven members from bundled public-domain / synthetic clips **and** a consented "your voice" target, plus the AI-onboarding/voice-catalog idea. **Still blocked for the *real-time* path:** as of 2026-06 there is no real-time, on-CPU, *permissively-licensed* zero-shot VC model — the fast CPU option (RT-VC, ~61 ms) ships no license, and the permissive ones (kNN-VC, FreeVC; both MIT) depend on WavLM-Large (~1.3 GB, GPU-oriented), which is the real-time killer on CPU. Revisit when a real-time, ONNX-friendly, permissively-licensed option matures (RT-VC is the one to watch).

> **Note (2026-06):** the *offline* version of this — accent-preserving cloning for **Speak** (not the live changer) — was researched separately and is **no longer blocked**: permissive, CPU-runnable, ONNX-friendly zero-shot TTS engines (VoxCPM, Chatterbox) now exist. See the spike row above + [docs/research/accent-preserving-cloning.md](research/accent-preserving-cloning.md). VC remains the wrong tool there (it preserves the *source* accent, not the user's); reference-conditioned TTS is the path.

**Next up:** the v1.8–v1.10 roadmap is shipped (dynamics, control surfaces, guided mic calibration), plus light theme (v1.11) and the update check + E2E harness (v1.12). Shipped v1.13 setup diagnostic and v1.14 quick wins (preset import, logs/report affordance, README roadmap). **Standout features** track: v1.15 custom glyph casting and v1.16 spell-circle stream overlay both shipped. Further work follows standard semver — see "Further out" below.

Further out (standard semver): VST3 host, OBS WebSocket, community preset registry, etc. (Light theme shipped — v1.11.0.)
...
- **Tag style:** annotated tags. The GitHub release body is generated by [`release.yml`](../.github/workflows/release.yml) — SHA-256 checksums plus GitHub’s auto-generated commit list — *not* lifted from CHANGELOG. The changelog reaches users through the in-app **What’s new** panel instead (v1.49.0).
