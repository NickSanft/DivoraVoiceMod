...
| post-1.0 | v1.7.0 | **Zero-shot conversion + personal voice** — *hybrid step 2.* Integrate an any-to-reference VC model (convert toward a reference clip, no per-voice training), which unlocks realistic Coven members from bundled public-domain / synthetic clips **and** a consented "your voice" target recorded in-app. Folds in the old AI-onboarding/voice-catalog idea (first-run "Enable AI voice?" + a download catalog). | Pick or record a reference voice → speak in it live; realistic cast members install without training |
| post-1.0 | v1.8.0 | **Loudness normalization / auto-gain**: optional RMS/LUFS-target stage + limiter so perceived level stays steady across presets + conversion | Switching presets or enabling Voice Convert keeps output loudness roughly constant without clipping |
