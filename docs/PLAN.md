...
|---|---|---|---|
| post-1.0 | v1.1.0 | **The Coven (voice cast)** — a browsable cast of original character voices (Velvet Demon, Hollow King, Choir of Ash, Static Wraith, The Oracle), each a tuned **DSP voice identity**, plus the existing LLVC narrator, unified under one "Voice browser." *Hybrid step 1.* Additive only: a bundled cast manifest (`coven.json`) + a `list_cast` command + (likely) a new `chorus`/doubler effect; "Summon" reuses `set_effect_chain` / `set_voice_model`. Introduces a `Voice` data model that covers both DSP- and model-backed voices so realistic ones (v1.3) drop in cleanly. | Open the Coven browser, click a character → your voice takes on that character live; casting the bound glyph summons the same |
| post-1.0 | v1.2.0 | **Streaming LLVC (low-latency AI)**: re-export LLVC in streaming mode (thread the encoder/decoder/convnet cache tensors between chunks), carry per-instance state through `run_inference`, shrink the chunk — so the AI voices are usable in live conversation, not just recordings | Voice Convert latency drops from ~256 ms toward <60 ms with no audible seams (verified via the Phase 14 readout) |
| post-1.0 | v1.3.0 | **Zero-shot conversion + personal voice** — *hybrid step 2.* Integrate an any-to-reference VC model (convert toward a reference clip, no per-voice training), which unlocks realistic Coven members from bundled public-domain / synthetic clips **and** a consented "your voice" target recorded in-app. Folds in the old AI-onboarding/voice-catalog idea (first-run "Enable AI voice?" + a download catalog). | Pick or record a reference voice → speak in it live; realistic cast members install without training |
| post-1.0 | v1.4.0 | **Loudness normalization / auto-gain**: optional RMS/LUFS-target stage + limiter so perceived level stays steady across presets + conversion | Switching presets or enabling Voice Convert keeps output loudness roughly constant without clipping |
