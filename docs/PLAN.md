...
| **14** | v0.14.0 | **Latency & buffer transparency**: compute end-to-end latency from the known contributors (device buffers, resampler delay, denoiser 10 ms frame, Voice Convert 256 ms chunk) and show a live readout; optional buffer-size selector exposing the latency↔dropout trade. Foundational instrumentation for streaming LLVC | A live "Latency: ~N ms" readout that moves when Voice Convert / denoiser toggle; picking a buffer size visibly changes it |
| **15** | v0.15.0 | **Record modulated output**: one-click capture of the post-chain output to WAV, with an indicator + a recordings folder. Reuses the existing output tap | Record → speak → stop yields a playable `.wav` of the modulated voice |
| **1.0** | v1.0.0 | **Stable surface cut** after a quiet 0.15.x cycle: no new features — freeze the preset JSON + Tauri command surface for back-compat, manual test pass, ship. 8 effects + soundboard + presets + cast + AI voice + monitor routing + latency + recording | API/preset format stable for back-compat; public "v1" release |

After v1.0, features follow standard semver:

| Rel. | Version | Theme | Success criterion |
|---|---|---|---|
| post-1.0 | v1.1.0 | **Streaming LLVC (low-latency AI)**: re-export LLVC in streaming mode (thread the encoder/decoder/convnet cache tensors between chunks), carry per-instance state through `run_inference`, shrink the chunk | Voice Convert latency drops from ~256 ms toward <60 ms with no audible seams (verified via the Phase 14 readout) |
| post-1.0 | v1.2.0 | **AI onboarding + voice catalog**: a first-run "Enable AI voice? (~30 MB)" download step + a Settings catalog of additional consent-respecting voices with download/progress | A new user is offered AI in the wizard and can browse + install extra voices |
| post-1.0 | v1.3.0 | **Loudness normalization / auto-gain**: optional RMS/LUFS-target stage + limiter so perceived level stays steady across presets + conversion | Switching presets or enabling Voice Convert keeps output loudness roughly constant without clipping |
