...
| post-1.0 ◐ | v1.17.0 | **Text-to-speech "Speak" (Phase 1 scaffolding)**: a new Speak section — type text → pick a preset **Kokoro-82M** voice → synthesize → play through the output, mixed with the mic via the soundboard seam. On-device (Kokoro via `ort`; **espeak-ng** phonemizer run as an arm's-length GPL subprocess so app code stays MIT). This commit lands the no-asset parts — pure tokenizer/chunker + phonemizer wrapper + `list_tts_voices`/`speak`/`stop_speak` + screen/store/nav — with **synthesis gated behind "voice not installed"** until the model assets are staged in `voice-assets-v2`. Additive `divora.ttsVoice`, `TtsVoiceInfo`. | Speak screen renders + picks a voice; pressing Speak shows a graceful "not installed" notice (no hang); synthesis activates once assets land **and** the GPL espeak-ng disclosure is in the README, then it's tagged |
